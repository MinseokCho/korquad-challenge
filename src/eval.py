...
def evaluate(args, model, eval_examples, eval_features):
    ...
    results = write_predictions(eval_examples, eval_features, all_results,
    ...
    performance = json.dumps(korquad_eval(dataset, predictions))
    logger.info(performance)
    with open(os.path.join(args.output_dir, 'performance.txt'), 'a') as fw:
        fw.write('%s\n'%performance)
...
def main():
    ...
    tokenizer = BertTokenizer(vocab_file=args.vocab, do_basic_tokenize=True, max_len=args.max_seq_length)
