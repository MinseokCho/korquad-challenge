...
class Model(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, adv_grad=None, cal_grad=False, is_gs_noise=False):
        ...
        self.embedding_output = self.embeddings(input_ids, token_type_ids)

        if adv_grad is not None:
            self.embedding_output += 0.05 * adv_grad.sign()
        elif is_gs_noise:
            gs_noise = torch.randn(self.embedding_output.size()).cuda()
            self.embedding_output += gs_noise
        elif cal_grad:
            self.embedding_output = self.embedding_output.clone().detach().requires_grad_(True)
        self.encoded_layers = self.encoder(self.embedding_output,
                                        extended_attention_mask)

        sequence_output = self.encoded_layers[-1]
...
class QuestionAnswering(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, adv_grad=None, cal_grad=False, is_gs_noise=False):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, adv_grad, cal_grad, is_gs_noise)
